...
def create_comparison_image(gt_img, recon_img, epoch, psnr_val, ssim_val, lpips_val):
    ...
    axes[0].set_title('Ground Truth (High-Res)', fontsize=12)
    axes[0].axis('off')
...
def train_model(args):
    ...
    # img_hr is the HIGH-RES ground truth (x_gt)
    img_hr = load_image(args.image) 
    h_hr, w_hr, c = img_hr.shape
 
    # coords_hr are the HIGH-RES coordinates (u_hr)
    coords_hr = get_coordinates(h_hr, w_hr)
    
    # Store GT image for final metric calculation
    gt_img_np = img_hr.numpy() # This is x_gt
    
    if args.task == 'super_resolution':
        print(f"Performing {args.scale_factor}x super-resolution task.")
        is_inpainting_task = False # Disable inpainting for SR task
        
        # 1. Define the downsampling operator A_scale
        scale = args.scale_factor
        h_lr, w_lr = h_hr // scale, w_hr // scale
        
        # We need image in (B, C, H, W) format for F.interpolate
        img_hr_bchw = img_hr.permute(2, 0, 1).unsqueeze(0).to(device)

        def downsampler(x_hr_bchw):
            """
            Applies the A_scale operator:
            Bicubic downsampling with antialiasing.
            """
            return F.interpolate(x_hr_bchw, size=(h_lr, w_lr), mode='bicubic', antialias=True)

        # 2. Create the LOW-RES target image (y)
        # We detach it: we don't need gradients flowing back into the *target*.
        target_lr_bchw = downsampler(img_hr_bchw).detach() # This is y

        # 3. Log the low-res image for visualization
        lr_vis = target_lr_bchw.squeeze(0).permute(1, 2, 0).cpu().numpy()
        wandb.log({"images/low_res_target": wandb.Image(np.clip(lr_vis, 0, 1), caption="Low-Res Target (y)")})

        # 4. Set the model's input coordinates (ALWAYS high-res)
        train_coords = coords_hr.to(device)
        
        # This 'train_target' isn't used in the SR loss, 
        # but we set it to the LR target flat shape for consistency
        # in case any code accidentally uses it.
        train_target = target_lr_bchw.squeeze(0).permute(1, 2, 0).view(-1, c).to(device)
        
    else: # Original fitting/inpainting logic
        target_hr_flat = img_hr.view(-1, c) # This is x_gt
        ...
        if is_inpainting_task:
            ...
            train_coords = coords_hr[train_indices].to(device)
            train_target = target_hr_flat[train_indices].to(device)
            
            test_coords = coords_hr[test_indices].to(device)
            test_target = target_hr_flat[test_indices].to(device)
            
            mask = torch.ones(num_pixels, 1) * 0.2 # just for visualization
            mask[train_indices] = 1.0
            mask_img = mask.view(h_hr, w_hr, 1).cpu().numpy()
            ...
            train_coords = coords_hr.to(device)
            train_target = target_hr_flat.to(device)
    ...
    for epoch in range(args.epochs):
        ...
        if args.optimizer == 'lbfgs':
            ...
            def closure():
                ...
                if args.task == 'super_resolution':
                    # 1. Get high-res prediction: f(u_hr) -> x_hat
                    pred_hr_flat = model(coords_hr.to(device))
                    pred_hr_bchw = pred_hr_flat.view(1, h_hr, w_hr, c).permute(0, 3, 1, 2)
                    
                    # 2. Downsample prediction: A_4(x_hat) -> y_hat
                    pred_lr_bchw = downsampler(pred_hr_bchw)
                    
                    # 3. Calculate loss: || y_hat - y ||^2
                    loss = F.mse_loss(pred_lr_bchw, target_lr_bchw)
                else: # Original fitting/inpainting loss
        ...
        if epoch % args.log_n_epochs == 0:
            model.eval()
            with torch.no_grad():
                full_pred = model(coords_hr.to(device))
                pred_img = full_pred.view(h_hr, w_hr, c).cpu().numpy() # Reconstructed x_hat
                target_img = gt_img_np # Ground truth x_gt
                ...
                # B. Calculate PSNR on train/test subsets (only for inpainting)
                train_psnr_val = 0.0
                test_psnr_val = 0.0
                if is_inpainting_task:
                    ...
                    # train_psnr is not relevant here, as loss is on LR.
                    train_psnr_val = full_psnr_val
                    print(f"Epoch {epoch:4d}: Loss(LR)={loss.item():.6f}, PSNR(HR)={full_psnr_val:.2f}, SSIM={ssim_val:.4f}, LPIPS={lpips_val:.4f}")
                else: # Overfitting task
                    train_psnr_val = full_psnr_val
                ...
                log_dict = {
                    'epoch': epoch,
                    'loss': loss.item(), # This is LR loss for SR, HR loss for fitting
                ...
                if is_inpainting_task:
                    log_dict['train_psnr'] = train_psnr_val
    ...
    # Return the high-res image
    return model, metrics, layer_metrics, img_hr, lpips_model

def plot_metrics_seaborn_separate(metrics, layer_metrics, args):
    """Plot training metrics using seaborn and log to wandb."""
    sns.set_style("darkgrid")
    
    is_inpainting_task = args.inpainting_ratio < 1.0 and args.task != 'super_resolution'
    ...
    if is_inpainting_task:
        main_df['Train PSNR'] = metrics['train_psnr']
        main_df['Test PSNR'] = metrics['test_psnr']

    # 1. PSNR Plot (handles both inpainting and overfitting)
    ...
    if is_inpainting_task:
        ...
        title = f'PSNR Over Training ({args.task.capitalize()})'
    ...
    # 2. SSIM and LPIPS plots (from your original function)
    plt.figure(figsize=(10, 6))
    ...
    for key, values in layer_metrics.items():
        if len(values) == len(metrics['epochs']):
            ...
            print(f"Skipping plot for {key}: mismatch in length.")
    ...
    # 4. Effective Ranks Plot  <-- ADD THIS ENTIRE BLOCK
    ...
    # 5. Spectral Norms Plot  <-- (Renumbered from 4)
    ...
    # 6. Condition Numbers Plot  <-- (Renumbered from 5)
...
def main():
    ...
    parser.add_argument('--log_n_epochs', default=200, type=int, help='Frequency of logging metrics and images.')
    ...
    parser.add_argument('--project_name', type=str, default='cvpr-sisr', help='Wandb project name.')
    ...
    parser.add_argument('--T_max', type=int, default=2000, help='T_max for CosineAnnealingLR scheduler.')
    ...
    parser.add_argument('--model', choices=['relu_mlp', 'relu_ffn', 'gauss_ffn', 'gauss_mlp', 'siren_mlp', 'wire_mlp', 'relu_pos_enc', 'real_wire', 'replicate_mlp', 'finer_mlp', 'fourier_net'], default='relu_ffn')
    parser.add_argument('--num_layers', default=4, type=int, help='Number of hidden layers in the MLP.')
    ...
    parser.add_argument('--task', type=str, default='super_resolution', choices=['fitting', 'super_resolution'], 
                        help='Task to perform: fitting (overfitting/inpainting) or super_resolution.')
    parser.add_argument('--scale_factor', type=int, default=4, 
                        help='Downsampling scale for super-resolution task.')
    parser.add_argument('--inpainting_ratio', default=1.0, type=float, help='Ratio of pixels for training (1.0 for overfitting, <1.0 for inpainting). Only used if task="fitting".')
    ...
    # original_img is the high-res image
    model, metrics, layer_metrics, original_img, lpips_model = train_model(args)
    
    # Final evaluation and logging
    model.eval()
    with torch.no_grad():
        h_hr, w_hr, c = original_img.shape
        coords_hr = get_coordinates(h_hr, w_hr).to(next(model.parameters()).device)
        final_pred = model(coords_hr).view(h_hr, w_hr, c).cpu()
